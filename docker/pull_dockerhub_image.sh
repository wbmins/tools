...
get_token() {
  local token_json
  token_json=$(curl -s "https://auth.docker.io/token?service=registry.docker.io&scope=repository:${REPO}:pull")
...
get_manifest() {
  ...
  if [[ "$KIND" == "list" ]]; then
    echo "检测到 multi-arch manifest list，尝试选择架构 $ARCH ..."
...
get_layers() {
  ...
  curl -sL -H "Authorization: Bearer $TOKEN" "https://registry-1.docker.io/v2/${REPO}/blobs/${CONFIG_DIGEST}" -o "$config_file"
}
...
EOF
  ...
  if [[ "$DO_LOAD" -eq 1 ]]; then
    ...
    echo "Done: docker load -i $OUTFILE"
  fi
}
